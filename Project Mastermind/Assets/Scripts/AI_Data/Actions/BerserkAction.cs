    ...
    public override bool perform(GameObject agent)
    {
        ...
        if (anim.GetBool("isInteracting") != true) //Did we start animating an action...
        {
            ...
            else                                              //...else do my action
            {
                ...
                SoundManager.PlaySound(SoundManager.Sound.EnemyBerserk, this.transform.position);
                HashSet<GoapAction> availableActions = goapC.GetAvailableActions();
                foreach (GoapAction a in availableActions)
                {
                    a.actionDamage += 10;               
                }
                Transform berserk = GameObject.Find("BerserkAI").transform;
                berserk.position = anim.transform.position;
                berserk.parent = anim.transform;
                Debug.Log("Berserk on AI. All actions received +5 dmg.");
            }
        }
        return true;
    }
